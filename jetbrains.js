...
function getPluginProductCodes() {
    ...
    for (let i = 0; i < 500; i++) {
        $.getJSON("https://plugins.jetbrains.com/api/searchPlugins", {
            ...
            for (let j = 0; j < response.plugins.length; j++) {
                if (response.plugins[j].pricingModel === "FREE") {
                    continue;
                }
                $.getJSON("https://plugins.jetbrains.com/api/plugins/" + response.plugins[j].id).then((response) => {
